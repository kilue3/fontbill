...
const Bill_detail_page = ({ id }) => {
  ...
  const newName = id + "_" + timestamp; //เปลี่ยนชื่อ
  const uploadFileToFirebase = (file) => {
    const uploadTask = storage.ref(`${id}/${newName}`).put(file); //firebase storeage
    //ref เลือกfolder
    uploadTask.on(
      "state_changed",
      (snapshot) => {
  ...
  const SUPPORTED_TYPE = ["image/jpeg", "image/png"];
      ...
      if (result.isConfirmed) {
        axios
          ...
          .then((res) => {
            if (res.data.message == "success") {
              ////ต่อตรงนี้
              Swal.fire("Deleted!", "ลบไฟล์สำเร็จ.", "success")
      ...
      if (result.isConfirmed) {
        axios
          ...
          .then((res) => {
            if (res.data.message == "success") {
              ////ต่อตรงนี้
              Swal.fire("Deleted!", "ยกเลิกบิลสำเร็จ.", "success")
              .then(() => window.location.assign("/Billuser"));
            } else {
                      ...
                      {bFile.map((files) => {
                        return (
                          <>
                            <tr>
                            <td>
                                <Button color="link" outline href={files.file_url}>{files.file_name}</Button>

                                </td>
                              <th>{files.file_date}</th>
                              <td>
                                {" "}
                                <Button color="danger"  onClick={() => alertdelect(files.id)} >ลบไฟล์</Button>
